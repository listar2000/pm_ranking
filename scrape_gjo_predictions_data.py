...
class GJOPredictionsScraper:
    ...
    def _get_sleep_time(self, base_time: float = 1.0) -> float:
        ...
        if self.retry_count == 0:
            ...
            backoff_time = base_time * (2 ** self.retry_count) + random.uniform(0, 2)
            ...
            while page <= max_pages:
                try:
                    ...
                    if not response or response.status != 200:
                        print(f"  Failed to load page {page}, status: {response.status if response else 'No response'}")
                        break
                    
                    # Wait for the content to load
                    await page_obj.wait_for_selector('.flyover-comment', timeout=10000)
                    ...
                    prediction_comments = soup.find_all('div', class_='flyover-comment rationale')
                    ...
                    for comment in prediction_comments:
                        prediction_data = self._extract_prediction_from_comment(comment, problem_id, problem_options)
                        if prediction_data:
                            page_predictions.append(prediction_data)
                    
                    all_predictions.extend(page_predictions)
                    print(f"  Found {len(page_predictions)} new predictions on page {page}")
                    ...
                    if self.retry_count <= self.max_retries:
                        print(f"  Retrying... (attempt {self.retry_count}/{self.max_retries})")
                        sleep_time = self._get_sleep_time(2.0)
                        await asyncio.sleep(sleep_time)
                        continue
                    else:
                        print(f"  Max retries reached, stopping for problem {problem_id}")
                        break
                    
        finally:
            await page_obj.close()
        
        print(f"Total predictions found for problem {problem_id}: {len(all_predictions)}")
    ...
    def _extract_prediction_from_comment(self, comment_element, problem_id: int, problem_options: List[str]) -> Optional[Dict]:
        ...
        try:
            # Extract username
            username_element = comment_element.find('span', class_='membership-username')
            ...
            # Extract timestamp
            timestamp_element = comment_element.find('span', attrs={'data-localizable-timestamp': True})
            timestamp = timestamp_element.get('data-localizable-timestamp') if timestamp_element else None
            
            # Extract prediction values as list
            prediction_values = self._extract_prediction_values_as_list(comment_element, problem_options)
    ...
    def _extract_prediction_values_as_list(self, comment_element, problem_options: List[str]) -> Optional[List[float]]:
        ...
        try:
            ...
            values_container = comment_element.find('div', class_='prediction-values')
            if not values_container:
                return None
            
            # Find all prediction rows
            prediction_rows = values_container.find_all('div', class_='row row-condensed')
            ...
            for row in prediction_rows:
                ...
                if probability_col and answer_col:
                    ...
                    probability_element = probability_col.find('span', class_='probability-value')
                    if probability_element:
                        probability_text = probability_element.get_text(strip=True)
                        # Remove % sign and convert to float
                        probability = float(probability_text.replace('%', '')) / 100.0
    ...
    async def get_predictions_for_all_problems(self, problems_file: str, max_pages_per_problem: int = 10) -> List[Dict]:
        """
        Extract predictions for all problems from the enriched problems JSON file.
        ...
        for i, problem in enumerate(problems):
            ...
            try:
                ...
                print(f"✓ Completed problem {problem_id}: {len(predictions)} predictions")
...
async def scrape_all_predictions():
    ...
    async with GJOPredictionsScraper() as scraper:
        ...
        try:
            all_predictions = await scraper.get_predictions_for_all_problems(problems_file, max_pages_per_problem=5)
            ...
            unique_users = len(set(p['username'] for p in all_predictions))
            unique_problems = len(set(p['problem_id'] for p in all_predictions))
            print(f"Unique users: {unique_users}")
            print(f"Unique problems: {unique_problems}")
            
        except FileNotFoundError:
            print(f"Problems file {problems_file} not found. Please run the problem scraper first.")
...
async def main():
    ...
    # await scrape_all_predictions()  # Scrape all problems
