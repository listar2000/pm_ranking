...
if __name__ == "__main__":
    ...
    market_earning_0 = MarketEarning(num_money_per_round=1, risk_aversion=0.0)
    market_earning_05 = MarketEarning(num_money_per_round=1, risk_aversion=0.5)
    market_earning_1 = MarketEarning(num_money_per_round=1, risk_aversion=1.0)

    results_0 = market_earning_0.fit_stream(prophet_arena_challenge.stream_problems(increment=50, order="time"))
    results_05 = market_earning_05.fit_stream(prophet_arena_challenge.stream_problems(increment=50, order="time"))
    results_1 = market_earning_1.fit_stream(prophet_arena_challenge.stream_problems(increment=50, order="time"))
    ...
    for i, (risk_aversion, results) in enumerate(zip([0.0, 0.5, 1.0], [results_0, results_05, results_1])):
        ...
        ax.set_title(f'Risk Aversion: {risk_aversion}', fontweight='bold', pad=20)
