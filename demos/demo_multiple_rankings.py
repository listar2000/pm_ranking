...
def demo_multiple_rankings():
    ...
    brier_result = brier_scoring_rule.fit(
        gjo_challenge.forecast_problems, include_scores=False)
    brier_rankings: Dict[str, int] = brier_result if isinstance(
        brier_result, dict) else brier_result[1]

    # cuda can be slower than cpu since we are running MCMC.
    device = "cuda" if torch.cuda.is_available() else "cpu"
    print(f"Using device: {device}")
    irt_model = IRTModel(n_bins=6, use_empirical_quantiles=False)
    svi_config = SVIConfig(optimizer="Adam", num_steps=3000,
                           learning_rate=0.005, device=device)
    irt_result = irt_model.fit(
        gjo_challenge.forecast_problems, method="SVI", config=svi_config)
    irt_rankings: Dict[str, int] = irt_result if isinstance(
        irt_result, dict) else irt_result[1]
